<?php   
	...
	class DisqusBlogIndexBlockController extends BlockController {

		protected $btTable = 'btDisqusBlogIndex';
		protected $btInterfaceWidth = "500";
		protected $btInterfaceHeight = "350";

		public function getBlockTypeDescription() {
			return 'List of pages for blog for "Disqus Comments" block.';
		}

		public function getBlockTypeName() {
			return "Disqus Blog List";
		}

		public function getJavaScriptStrings() {
			return array(
				'feed-name' => 'Type name of RSS feed.'
			);
		}
		...
		function getPages($query = null) {
			...
			$bID = $this->bID;
			if ($this->bID) {
				$q = "select num, cParentID, cThis, orderBy, ctID, displayAliases, rss from btDisqusBlogIndex where bID = '$bID'";
				$r = $db->query($q);
				if ($r) {
					$row = $r->fetchRow();
				}
		...
		*/
		...
		public function add() {
			...
			$this->set('c', $c);
			$this->set('uh', $uh);
			$this->set('bt', BlockType::getByHandle('disqus_blog_index'));
		...
		function save($args) {
			...
			$args['disqus_shortname'] = trim($args['disqus_shortname']);
			$args['protocol'] = isset($args['protocol']) ? intval($args['protocol']) : 0;
